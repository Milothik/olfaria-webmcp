...
export class OlfariaAtlas {
  ...
  setData(nodes, relations) {
    ...
    this.relationCounts = new Map(nodes.map((node) => [node.id, 0]));
    for (const relation of relations) {
      this.relationCounts.set(relation.source, (this.relationCounts.get(relation.source) || 0) + 1);
      this.relationCounts.set(relation.target, (this.relationCounts.get(relation.target) || 0) + 1);
    }
  ...
  rebuild(params) {
    ...
    const scored = scoped.map((node) => ({
      node,
      score: (this.activeIds.get(node.id) || 0) * 1000 + creativeValue(node) * 100 + (this.relationCounts.get(node.id) || 0),
    }))
  ...
  bind() {
    ...
    this.canvas.addEventListener('pointermove', (event) => {
      if (this.dragging) {
        ...
        this.showTooltip(point.x, point.y);
  ...
  project(point) {
    ...
    const base = Math.min(this.width, this.height) * (mobileView ? 0.42 : 0.44) * this.zoom;
...
function drawOlfemaCrystal(ctx, node, item, stateFlags) {
  ...
  if (active || selected || hovered || neighbor || relatedOnly) {
    ...
    ctx.fillStyle = selected
      ...
      : active
        ? `rgba(155,216,198,${0.08 + pulse * 0.035 + activationProgress * 0.025})`
        : relatedOnly
          ? 'rgba(216,188,130,0.055)'
          : 'rgba(155,216,198,0.09)';
    ctx.fill();
  }
  ...
  drawFacetedCrystalCore(ctx, node, coreRadius * 1.28, {
    hot,
    relatedOnly,
    lineWidth: selected ? 1.2 : active ? 0.95 + pulse * 0.12 : relatedOnly ? 0.5 : 0.72,
  });
  ctx.restore();
}

function drawCompactSnowflake(ctx, node, outerRadius, coreRadius, density, tension, hot) {
  const branchAlpha = hot ? 0.68 : 0.29;
  const armLength = Math.max(4.5, outerRadius * (0.72 + density * 0.18));
  const armWidth = Math.max(0.48, armLength * 0.045);
  ctx.beginPath();
  for (let i = 0; i < 6; i += 1) {
    const angle = -Math.PI / 2 + i * Math.PI / 3;
    ...
    const px = -sa * armWidth;
    const py = ca * armWidth;
    const startX = ca * coreRadius * 0.55;
    const startY = sa * coreRadius * 0.55;
    ctx.moveTo(startX + px, startY + py);
    ctx.lineTo(ca * end, sa * end);
    ctx.lineTo(startX - px, startY - py);
    ctx.closePath();
  }
  ctx.fillStyle = crystalTone(node.hedonicPolarity, 'base', branchAlpha * 0.48);
  ...
  ctx.strokeStyle = crystalTone(node.hedonicPolarity, 'light', branchAlpha * 0.62);
  ctx.lineWidth = 0.34;
  ctx.stroke();

  const compactCore = Math.max(1.3, coreRadius);
  ctx.beginPath();
  for (let i = 0; i < 6; i += 1) {
    const angle = Math.PI / 6 + i * Math.PI / 3;
    const x = Math.cos(angle) * compactCore;
    const y = Math.sin(angle) * compactCore;
    if (i) ctx.lineTo(x, y);
    else ctx.moveTo(x, y);
  }
  ctx.closePath();
  ctx.fillStyle = crystalTone(node.hedonicPolarity, 'light', 0.5);
  ctx.strokeStyle = 'rgba(248,251,250,0.34)';
  ctx.lineWidth = 0.4;
  ctx.fill();
  ctx.stroke();
}
...
function drawSnowflakeArm(ctx, descriptors, angle, length, coreRadius, node, axisIndex, hot, density, tension) {
  ...
  ctx.save();
  ctx.rotate(angle);
  drawCrystalShard(ctx, coreRadius * 0.82, 0, length, 0, Math.max(1.1, length * (hot ? 0.075 : 0.055)), {
    fill: crystalTone(node.hedonicPolarity, 'base', hot ? 0.26 : 0.15),
    stroke: crystalStroke(node.hedonicPolarity, hot ? 0.68 : 0.38),
    lineWidth: hot ? 0.92 : 0.56,
  });
  ...
  for (let index = 0; index < branchCount; index += 1) {
    ...
    [-1, 1].forEach((side) => {
      ...
      const branchAlpha = 0.2 + (descriptor.volatility || 0.4) * (hot ? 0.42 : 0.28);
      drawCrystalShard(ctx, x, 0, bx, by, Math.max(0.62, branch * (hot ? 0.075 : 0.055)), {
        fill: crystalTone(polarity, side > 0 ? 'light' : 'shadow', branchAlpha * 0.55),
        stroke: crystalStroke(polarity, branchAlpha),
        lineWidth: (hot ? 0.52 : 0.36) + weight * 0.24,
      });
      if (hot && index < 3) {
        const twig = branch * 0.34;
        drawCrystalShard(ctx, bx, by, bx - Math.cos(side * tilt * 1.65) * twig, by + Math.sin(side * tilt * 1.65) * twig, Math.max(0.44, twig * 0.08), {
          fill: crystalTone(polarity, 'light', 0.11),
          stroke: crystalStroke(polarity, 0.24),
          lineWidth: 0.34,
        });
      }
    });
...
function drawCrystalShard(ctx, x1, y1, x2, y2, width, style) {
  const dx = x2 - x1;
  const dy = y2 - y1;
  const length = Math.hypot(dx, dy) || 1;
  const px = -dy / length * width;
  const py = dx / length * width;
  const shoulderX = x1 + dx * 0.82;
  const shoulderY = y1 + dy * 0.82;
  ctx.beginPath();
  ctx.moveTo(x1 + px, y1 + py);
  ctx.lineTo(shoulderX + px * 0.48, shoulderY + py * 0.48);
  ctx.lineTo(x2, y2);
  ctx.lineTo(shoulderX - px * 0.48, shoulderY - py * 0.48);
  ...
  ctx.moveTo(x1, y1);
  ctx.lineTo(x2, y2);
  ctx.strokeStyle = 'rgba(247,250,249,0.22)';
  ctx.lineWidth = Math.max(0.28, style.lineWidth * 0.42);
  ctx.stroke();
}

function drawFacetedCrystalCore(ctx, node, radius, options = {}) {
  ...
  points.forEach((point, index) => {
    ...
    ctx.closePath();
    ctx.fillStyle = crystalTone(node.hedonicPolarity, index % 3 === 0 ? 'light' : index % 2 ? 'shadow' : 'base', options.hot ? 0.7 : 0.46);
    ctx.fill();
  });
  ctx.beginPath();
  points.forEach((point, index) => index ? ctx.lineTo(point[0], point[1]) : ctx.moveTo(point[0], point[1]));
  ctx.closePath();
  ctx.strokeStyle = options.hot ? 'rgba(247,250,249,0.86)' : options.relatedOnly ? 'rgba(216,188,130,0.46)' : crystalStroke(node.hedonicPolarity, 0.58);
  ctx.lineWidth = options.lineWidth || 0.65;
  ctx.stroke();
  ctx.beginPath();
  ctx.arc(-radius * 0.16, -radius * 0.14, Math.max(0.35, radius * 0.16), 0, Math.PI * 2);
  ctx.fillStyle = 'rgba(255,255,255,0.72)';
  ctx.fill();
}

function crystalTone(polarity, tone, alpha) {
  if (polarity < -0.25) {
...
function edgeColor(edge, hot, alpha) {
  ...
  if (edge.relationClass === 'risk' || edge.relationClass === 'negative') return `rgba(205,136,112,${a})`;
  if (edge.relationClass === 'creative') return `rgba(216,188,130,${a})`;
  if (edge.relationClass === 'neutral' || edge.relationClass === 'unknown') return `rgba(220,227,224,${a * 0.82})`;
  return `rgba(155,216,198,${a})`;
}
